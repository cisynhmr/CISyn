...
def smplx_pose_to_sim_dofs(pose_smplx, trans=None, ori=None):
    ...
    root_euler = np.array(ori, dtype=np.float64)
    root_euler[0] += 90
...
def process(meta, seq_dir, num_steps=480, fps_target=120.0, slowdown=4, render=True, video_fps=30):
    """
    Run Genesis simulation from meta (from seq_define) and save amass_0.npz, amass_1.npz to seq_dir.
    ...
    # Set DOFs from meta: use pose1/pose2 (SMPLX) if present, else pos/ori only
    if "pose1" in meta and "pose2" in meta:
        ...
        n1, n2 = humanoid1.n_dofs, humanoid2.n_dofs
        if len(dofs1) != n1:
    ...
    a1 = meta['asset1'].split('/')[-2]
    a2 = meta['asset2'].split('/')[-2]
    name1, name2 = (f"{a1}_1.npz", f"{a2}_2.npz") if a1 == a2 else (f"{a1}.npz", f"{a2}.npz")
    np.savez(os.path.join(seq_dir, name1), **make_amass(poses_h1, trans_h1, gender1, betas1, root_pos1, pelvis_pos1))
    np.savez(os.path.join(seq_dir, name2), **make_amass(poses_h2, trans_h2, gender2, betas2, root_pos2, pelvis_pos2))
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Run Genesis simulation for a single sequence folder.")
