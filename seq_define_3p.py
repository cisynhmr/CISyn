...
Lookat: around (0, 0, 1.0-1.5).
FOV stored as horizontal FOV (what xrfeitoria expects).
return position, lookat, fov_horizontal, resolution
"""
def random_camera():
    ...
    x = r * math.cos(angle)
    y = r * math.sin(angle)
    z = 1.5 + random.uniform(-0.2, 0.5)

    lookat_x = random.uniform(-0.3, 0.3)
    lookat_y = random.uniform(-0.3, 0.3)
    lookat_z = random.uniform(1.2, 1.6)

    # Convert longer-side FOV to horizontal FOV for xrfeitoria
    if h > w:
        # Portrait: longer side is vertical; convert vertical -> horizontal
        fov_h = math.degrees(2 * math.atan(math.tan(math.radians(fov_long / 2)) * w / h))
    else:
...
for human 3, the "angle3" cannot be in angle1 or angle2 +- 48 degrees
return pos1, pos2, pos3, ori1, ori2, ori3
"""
def random_human():
    radius = 2.4
    z = 1.5
    ...
    while True:
        ...
        diff1 = abs((angle3 - angle1 + math.pi) % (2 * math.pi) - math.pi)
        diff2 = abs((angle3 - angle2 + math.pi) % (2 * math.pi) - math.pi)
        if math.degrees(diff1) >= 48 and math.degrees(diff2) >= 48:
            break
    pos1 = (radius * math.cos(angle1), radius * math.sin(angle1), z)
    pos2 = (radius * math.cos(angle2), radius * math.sin(angle2), z)
    pos3 = (radius * math.cos(angle3), radius * math.sin(angle3), z)
    ...
    ori3 = random_euler_deg()
    return pos1, pos2, pos3, ori1, ori2, ori3


"""
Random velocity for the three humans.
...
def random_vel(pos1, pos2, pos3):
    def vel_for_pos(pos):
        ...
        return (vx, vy, vz)
    return vel_for_pos(pos1), vel_for_pos(pos2), vel_for_pos(pos3)


"""
Random angular velocity (rad/s) for the three humans.
...
def main():
    ...
    pos1, pos2, pos3, ori1, ori2, ori3 = random_human()
    vel1, vel2, vel3 = random_vel(pos1, pos2, pos3)
    angvel1, angvel2, angvel3 = random_angular_vel()

    asset1_idx = random.randint(0, len(asset_list) - 1)
    asset2_idx = random.randint(0, len(asset_list) - 1)
    while asset2_idx == asset1_idx:
        asset2_idx = random.randint(0, len(asset_list) - 1)
    asset3_idx = random.randint(0, len(asset_list) - 1)
    while asset3_idx == asset1_idx or asset3_idx == asset2_idx:
        asset3_idx = random.randint(0, len(asset_list) - 1)
    pose1_idx = random.randint(0, len(amass) - 1)
    pose2_idx = random.randint(0, len(amass) - 1)
    while pose2_idx == pose1_idx:
        pose2_idx = random.randint(0, len(amass) - 1)
    pose3_idx = random.randint(0, len(amass) - 1)
    while pose3_idx == pose1_idx or pose3_idx == pose2_idx:
        pose3_idx = random.randint(0, len(amass) - 1)
    meta = {
        "camera": cam_pos,
        "lookat": lookat,
