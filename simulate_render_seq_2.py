"""
Render a simulated sequence with xrfeitoria (2/3-person version).
Expects simulate_seq_2.py to have run first; meta (from seq_define_2) and results live in seq_dir.
n_actors = meta.get("n_actors", 2): 2 → two-person, 3 → three-person.
"""
import os
import subprocess
...
def _asset_folder(asset_path: str) -> str:
    """From meta['asset1/2/3'] (e.g. 'data/0000001/humanoid.xml') return folder name (e.g. '0000001')."""
...
def process(
    ...
    *,
    ...
    """
    Render the sequence for one seq folder using meta (from seq_define_2) and motion npz from simulate_seq_2.py.

    meta: dict from seq_define_2 (camera, asset1, asset2, [asset3], hdri, n_actors, ...).
          simulate_seq_2.py must have been run so that seq_dir contains the motion .npz files.
    seq_dir: folder where simulate_seq_2.py wrote meta.json and the motion .npz files.
    ...
    n_actors = int(meta.get("n_actors", 2))
    ...
    if not npz_path1.exists():
        raise FileNotFoundError(f"Motion npz not found: {npz_path1}. Run simulate_seq_2.py first.")
    if not npz_path2.exists():
        raise FileNotFoundError(f"Motion npz not found: {npz_path2}. Run simulate_seq_2.py first.")
    if n_actors >= 3:
        npz_name3 = f"{_asset_folder(meta['asset3'])}.npz"
        npz_path3 = seq_dir / npz_name3
        if not npz_path3.exists():
            raise FileNotFoundError(f"Motion npz not found: {npz_path3}. Run simulate_seq_2.py first.")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Render a single sequence folder with xrfeitoria (2/3-person).")
