...
class SMPLXMotion(Motion):
    ...
    def from_smplx_data(
        ...
        def vec_amass2smplx(vector: np.ndarray) -> np.ndarray:
            """
            ...
            if vector.shape == (3,):
                ret = np.array([vector[0], vector[2], -vector[1]], dtype=vector.dtype)
            elif vector.ndim == 2 and vector.shape[1] == 3:
                ret = np.array([vector[:, 0], vector[:, 2], -vector[:, 1]]).T
            else:
                raise ValueError(f'vector.shape={vector.shape}')
            return ret
        
        vector_convertor = vec_amass2smplx
        if vector_convertor is not None:
            transl_bl = vector_convertor(transl_bl)
    ...
    def from_amass_data(cls, amass_data, insert_rest_pose: bool, flat_hand_mean: bool = True) -> 'SMPLXMotion':
        ...
        betas = amass_data['betas'][:10]
        root_pos = amass_data['root_location_t0']
        pelvis_pos = amass_data['pelvis_location_t0']
        pelvis_ori_pos = pelvis_pos - root_pos
        # pelvis_ori_pos = np.array([pelvis_ori_pos[0], pelvis_ori_pos[2], -pelvis_ori_pos[1]])
        pelvis_pos_converted = np.array([pelvis_pos[0], -pelvis_pos[2], pelvis_pos[1]])
        transl = amass_data['trans']
        transl += pelvis_ori_pos
        transl -= pelvis_pos_converted
        transl *= 100
        # transl[:, 2] -= 120
        ...
        # motions in AMASS dataset are z up, rotate it to -y up
        # this will be multiplied with [180, 0, 0] later to make -y up to y up
        amass2humandata_adj = spRotation.from_euler('xyz', np.deg2rad([90, 0, 0]))
        global_orient = (amass2humandata_adj * spRotation.from_rotvec(global_orient)).as_rotvec()  # type: ignore
        # transl_0 = transl[0, :]
        # transl = amass2humandata_adj.apply(transl - transl_0) + transl_0
        # transl = ConverterMotion.vec_amass2humandata(transl)
