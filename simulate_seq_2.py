...
def process(meta, seq_dir, num_steps=480, fps_target=120.0, slowdown=4, render=True, video_fps=30):
    """
    Run Genesis simulation from meta (from seq_define_2) and save two (or three) .npz files to seq_dir.
    ...
    if "pose1" in meta and "pose2" in meta:
        ...
        if n_actors >= 3 and "pose3" in meta:
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Run Genesis simulation for a single sequence folder (2/3-person).")
