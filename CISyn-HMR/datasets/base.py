...
class BASE(Dataset):
    ...
    def process_img(self, img, meta_data, rot = 0., flip = False, scale = 1.0):
        # force aspect ratio via center zero-padding
        if self.force_aspect_ratio is not None:
            target_h, target_w = self.force_aspect_ratio
            target_ratio = target_h / target_w
            h, w = img.shape[:2]
            curr_ratio = h / w
            if curr_ratio < target_ratio:          # image too wide → pad height
                new_h = round(w * target_ratio)
                pad_top = (new_h - h) // 2
                pad_bottom = new_h - h - pad_top
                img = np.pad(img, ((pad_top, pad_bottom), (0, 0), (0, 0)))
                if 'cam_intrinsics' in meta_data:
                    meta_data['cam_intrinsics'][:, 1, 2] += pad_top   # cy
            elif curr_ratio > target_ratio:        # image too tall → pad width
                new_w = round(h / target_ratio)
                pad_left = (new_w - w) // 2
                pad_right = new_w - w - pad_left
                img = np.pad(img, ((0, 0), (pad_left, pad_right), (0, 0)))
                if 'cam_intrinsics' in meta_data:
                    meta_data['cam_intrinsics'][:, 0, 2] += pad_left  # cx
    ...
    def get_boxes(self, meta_data):
        ...
        if pnum == 0:
            meta_data['boxes'] = torch.zeros(0, 4)
            return
    ...
    def compute_joint_occlusion(self, meta_data):
        """GT inter-person joint occlusion via depth-ordered vertex silhouette projection.

        For each person, marks all num_kpts joints occluded by any closer person's mesh.
        Covers both the 24 SMPL body joints and the 21 extra regressor joints (indices 24-44),
        since all contribute L1 gradients and need consistent occlusion suppression.
        Adds 'j_occ_gt' (pnum, num_kpts) float32 tensor: 1.0 = occluded by a closer person.
        Follows the same vertex-projection pattern as gen_scale_map in utils/map.py.
        """
        pnum = meta_data['pnum']
        if pnum < 2:
            meta_data['j_occ_gt'] = torch.zeros(pnum, self.num_kpts, dtype=torch.float32)
            return

        verts  = meta_data['verts']   # (pnum, 6890, 3) camera-space
        j2ds   = meta_data['j2ds']    # (pnum, num_kpts, 2) all joints already projected
        depths = meta_data['depths'][:, 0]       # (pnum,) absolute depth of each root
        cam_K  = meta_data['cam_intrinsics']     # (pnum,3,3) or (1,3,3)
        H, W   = int(meta_data['img_size'][0]), int(meta_data['img_size'][1])

        # Project verts to 2D — identical formula to project_joints and gen_scale_map
        v2ds_homo = torch.matmul(verts, cam_K.transpose(-1, -2))  # broadcasts (1,3,3) correctly
        v2ds = v2ds_homo[..., :2] / v2ds_homo[..., 2:3]           # (pnum, 6890, 2)

        GRID = 4  # downsample: 1288 → 162-cell grid, suitable for body-scale occlusion
        map_h, map_w = H // GRID + 1, W // GRID + 1

        sorted_idx = torch.argsort(depths)      # ascending: closest person first
        occ_map  = np.zeros((map_h, map_w), dtype=np.uint8)
        j_occ_gt = torch.zeros(pnum, self.num_kpts, dtype=torch.float32)
        kernel   = np.ones((5, 5), dtype=np.uint8)  # 5×5 dilation fills vertex gaps at GRID=8

        for rank in range(len(sorted_idx)):
            idx = sorted_idx[rank].item()

            # 1. Check this person's joints against occ_map (union of all closer persons so far)
            if rank > 0:
                jx = (j2ds[idx, :, 0] / GRID).long().clamp(0, map_w - 1).numpy()
                jy = (j2ds[idx, :, 1] / GRID).long().clamp(0, map_h - 1).numpy()
                j_occ_gt[idx] = torch.from_numpy(occ_map[jy, jx].astype(np.float32))

            # 2. Add this person's vertex silhouette to occ_map for subsequent farther persons
            vx = (v2ds[idx, :, 0] / GRID).long().numpy()
            vy = (v2ds[idx, :, 1] / GRID).long().numpy()
            valid = (vx >= 0) & (vx < map_w) & (vy >= 0) & (vy < map_h)
            pmask = np.zeros((map_h, map_w), dtype=np.uint8)
            pmask[vy[valid], vx[valid]] = 1
            pmask = cv2.dilate(pmask, kernel, iterations=1)
            occ_map = np.maximum(occ_map, pmask)

        meta_data['j_occ_gt'] = j_occ_gt
    ...
    def process_data(self, img, raw_data, rot = 0., flip = False, scale = 1., crop = False):
        ...
        if self.human_type == 'smpl':
            self.compute_joint_occlusion(meta_data)
        ...
        if self.use_sat and meta_data['pnum'] > 0:
            # scale map
    ...
    def visualize(self, results_save_dir = None, vis_num = 100, vis_filter=None, vis_categories = ['verts', 'boxes', 'scale_map', 'j_occ_gt']):
        if results_save_dir is None:
            kid_suffix = '_usekid' if self.use_kid else ''
            results_save_dir = os.path.join('./datasets_visualization',f'{self.ds_name}_{self.split}{kid_suffix}')
        ...
        if vis_filter is not None and isinstance(vis_filter, str):
            vis_filter = [vis_filter]

        for idx in tqdm(range(len(self))):
            if idx % vis_interval != 0 and vis_filter is None:
                continue

            norm_img, targets = self.__getitem__(idx)
            if vis_filter is not None and not any(f in targets['img_path'] for f in vis_filter):
                continue
            ...
            if 'j2ds' in targets and ('j2ds_mask' in targets or 'jvis' in targets):
                gt_img = ori_img.copy()
                j2ds = targets['j2ds']
                if 'j2ds_mask' in targets:
                    j2ds_mask = targets['j2ds_mask']
                    normalized = False
                else:
                    j2ds_mask = targets['jvis'].unsqueeze(-1).expand_as(j2ds)
                    normalized = True
            ...
            if 'j_occ_gt' in targets and 'j2ds' in targets and 'j_occ_gt' in vis_categories:
                # Green = visible joint, Red = occluded joint (per j_occ_gt).
                # Draws all num_kpts joints (default 45: 24 SMPL body + 21 extra regressors).
                gt_img = ori_img.copy()
                n_occ_j = targets['j_occ_gt'].shape[1]
                j2ds    = targets['j2ds'][:, :n_occ_j, :]  # (pnum, num_kpts, 2)
                j_occ   = targets['j_occ_gt']               # (pnum, num_kpts)
                colors  = get_colors_rgb(len(j2ds))     # one distinct color per person
                for person_j2ds, person_occ, color in zip(j2ds, j_occ, colors):
                    bgr = (int(color[2]*255), int(color[1]*255), int(color[0]*255))
                    for kpt, occ in zip(person_j2ds, person_occ):
                        kpt_int = kpt.numpy().astype(int)
                        if occ > 0.5:
                            # Occluded: filled red circle with person-color ring
                            cv2.circle(gt_img, kpt_int, 5, (0, 0, 200), -1)
                            cv2.circle(gt_img, kpt_int, 6, bgr, 1)
                        else:
                            # Visible: filled green circle with person-color ring
                            cv2.circle(gt_img, kpt_int, 4, (0, 200, 0), -1)
                            cv2.circle(gt_img, kpt_int, 5, bgr, 1)
                cv2.imwrite(os.path.join(results_save_dir, f'{idx}_{img_name}_occ_gt.png'), np.hstack([ori_img, gt_img]))
