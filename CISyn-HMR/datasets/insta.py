...
class INSTA(BASE):
    def __init__(self, split='train', downsample=1, use_kid=True, **kwargs):
        super(INSTA, self).__init__(use_kid=use_kid, **kwargs)
        ...
        if not self.use_kid:
            kid_beta_thresh = 0.01
            self.annots = {
                k: v for k, v in self.annots.items()
                if not any(
                    len(p['smpl_param']['shape']) == 11 and abs(p['smpl_param']['shape'][10]) >= kid_beta_thresh
                    for p in v
                )
            }
    ...
    def get_raw_data(self, idx):
        ...
        for i in range(pnum):
            ...
            elif len(betas) == 11 and not self.use_kid:
                betas = betas[:10]
