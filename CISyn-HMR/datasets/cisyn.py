...
class CISyn(BASE):
    """CISyn dataset - loaded from cisyn_smpl_{split}.npz"""
    def __init__(self, split='train', downsample=1, **kwargs):
        super(CISyn, self).__init__(**kwargs)
        assert split in ['train', 'val', 'test', 'train_all']

        self.ds_name = 'cisyn'
        self.split = split
        self.dataset_path = os.path.join(dataset_root, 'cisyn')
        self.annots_path = os.path.join(self.dataset_path, 'cisyn_smpl_{}.npz'.format(split))
