...
def compute_interpen_loss(verts, faces, subsample=4):
    """Compute mean penetration distance for n people in camera space.

    Args:
        verts:     (n, V, 3) float tensor, camera-space vertices for n people.
                   Must be on the same device as faces.
        faces:     (F, 3) long tensor of face indices.
        subsample: stride for subsampling query points (default 8 → ~861 pts).
                   The mesh itself stays at full resolution for accuracy.
    Returns:
        Scalar tensor — mean sqrt(sq_dist) over all penetrating vertices across
        all pairs.  Returns 0. (on verts.device) when n < 2 or no penetration.
    """
    from kaolin.ops.mesh import check_sign, index_vertices_by_faces
    from kaolin.metrics.trianglemesh import point_to_mesh_distance
    n = verts.shape[0]
    if n < 2:
        return torch.tensor(0., device=verts.device)
    # Subsample query points; mesh stays full-resolution for check_sign / p2m.
    pts = verts[:, ::subsample, :]  # (n, V', 3)
    all_losses = []
    for i in range(n):
        for j in range(i + 1, n):
            v_i, v_j   = verts[i:i+1], verts[j:j+1]    # full mesh
            p_i, p_j   = pts[i:i+1],   pts[j:j+1]      # subsampled query pts
            with torch.no_grad():
                inside_ij = check_sign(v_j, faces, p_i).squeeze(0)  # (V',) bool
                inside_ji = check_sign(v_i, faces, p_j).squeeze(0)
            if inside_ij.any():
                sq_d, _, _ = point_to_mesh_distance(
                    p_i[:, inside_ij], index_vertices_by_faces(v_j, faces))
                loss_val = sq_d.clamp(min=1e-6).sqrt().mean()
                if torch.isfinite(loss_val):
                    all_losses.append(loss_val)
            if inside_ji.any():
                sq_d, _, _ = point_to_mesh_distance(
                    p_j[:, inside_ji], index_vertices_by_faces(v_i, faces))
                loss_val = sq_d.clamp(min=1e-6).sqrt().mean()
                if torch.isfinite(loss_val):
                    all_losses.append(loss_val)
    if not all_losses:
...
def _save_debug_ply(verts, faces):
    """Save n human meshes to ./tmp/interpen_debug.ply (colored by person index)."""
    import os, trimesh, numpy as np
    os.makedirs('./tmp', exist_ok=True)
    colors = [(255, 100, 100, 200), (100, 100, 255, 200), (100, 255, 100, 200),
              (255, 255, 100, 200), (255, 100, 255, 200)]
    meshes = []
    n = verts.shape[0]
    for i in range(n):
        v = verts[i].numpy()
        ...
        m.visual.vertex_colors = np.tile(c, (len(v), 1))
        meshes.append(m)
    combined = trimesh.util.concatenate(meshes)
    combined.export('./tmp/interpen_debug.ply')
    print(f'[DEBUG interpen] Saved {n} human meshes to ./tmp/interpen_debug.ply')
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, matcher, weight_dict, losses = ['confs','boxes', 'poses','betas', 'j3ds','j2ds', 'depths', 'kid_offsets'],
                focal_alpha=0.25, focal_gamma = 2.0, j2ds_norm_scale = 518, detect_all_mask_lvl = 'image', use_kid=False, smpl_faces=None,
                use_j_occ_gt=False):
        ...
        self.use_j_occ_gt = use_j_occ_gt
        if smpl_faces is not None:
            import numpy as np
            self.register_buffer('smpl_faces', torch.from_numpy(smpl_faces.astype(np.int64)))
        else:
    ...
    def loss_L1(self, loss, outputs, targets, indices, num_instances, **kwargs):
        ...
        if loss == 'j3ds':
            ...
            # Clamp to at most 54 joints; in practice num_kpts=45 so this is a no-op
            src = src[:,:54,:]
            target = target[:,:54,:]
        elif loss == 'j2ds':
        ...
        # Per-joint weighting for j3ds / j2ds.
        # Step 1 — GT occlusion: suppress joints geometrically occluded by a closer person.
        #          j_occ_gt (pnum, 24) float: 1.0 = occluded → weight = 0.1.
        # Step 2 — K&G heteroscedastic uncertainty (Laplacian NLL):
        #          pred_sigma (B, nq, 24) log_sigma; modulates loss when use_uncertainty=True.
        #          L_j = sqrt(2) * occ_w_j * |pred-gt|_j * exp(-s_j) + s_j
        if loss in ('j3ds', 'j2ds'):
            # import ipdb; ipdb.set_trace()
            N, J = valid_loss.shape[0], valid_loss.shape[1]

            # -- GT occlusion weight --
            occ_weight = torch.ones(N, J, device=src.device)
            if self.use_j_occ_gt and any('j_occ_gt' in t for t in targets):
                occ_gt = torch.cat(
                    [t['j_occ_gt'][i] for t, (_, i) in zip(targets, indices) if 'j_occ_gt' in t],
                    dim=0
                ).to(src.device)                     # (N, num_kpts)
                n_occ = occ_gt.shape[1]
                occ_weight[:, :n_occ] = 1.0 - occ_gt * 0.9  # range [0.1, 1.0]

            # -- K&G uncertainty: covers all num_sigma_joints (=num_kpts=45 by default) --
            if 'pred_sigma' in outputs:
                sigma = outputs['pred_sigma'][idx][valid_idx]   # (N, 24)
                n_s = min(sigma.shape[1], J)
                s = sigma[:, :n_s]                             # (N, n_s)
                # Laplacian NLL: sqrt(2)*occ_w*|pred-gt|*exp(-s) + s/D  (D=coord dims)
                D = valid_loss.shape[-1]
                kg = (math.sqrt(2) * occ_weight[:, :n_s].unsqueeze(-1)
                      * valid_loss[:, :n_s, :] * torch.exp(-s).unsqueeze(-1)
                      + s.unsqueeze(-1) / D)
                rest = valid_loss[:, n_s:, :] * occ_weight[:, n_s:].unsqueeze(-1)
                valid_loss = torch.cat([kg, rest], dim=1)
            else:
                valid_loss = valid_loss * occ_weight.unsqueeze(-1)
    ...
    def loss_interpenetration(self, loss, outputs, targets, indices, num_instances, **kwargs):
        assert loss == 'interpen'

        if 'pred_verts' not in outputs or self.smpl_faces is None:
            return {'interpen': torch.tensor(0., device=self.device)}

        pred_verts = outputs['pred_verts']  # (bs, nq, 6890, 3)
        # Detach translation so gradients only flow through pose/shape, not cam_head.
        pred_transl = outputs['pred_transl'].view(*pred_verts.shape[:2], 1, 3)
        pred_verts = pred_verts - pred_transl + pred_transl.detach()
        faces = self.smpl_faces.to(pred_verts.device)

        all_pen_losses = []
        for b_idx, (src_i, _) in enumerate(indices):
            src_i = [i for i in src_i if outputs['pred_confs'][b_idx, i] > 0.5]
            if len(src_i) != 2:
                continue
            # import ipdb; ipdb.set_trace()
            # _save_debug_ply(pred_verts[b_idx, src_i].detach().cpu(), faces.cpu())
            pen = compute_interpen_loss(pred_verts[b_idx, src_i], faces)
            if pen.item() > 0.:
                all_pen_losses.append(pen)

        if not all_pen_losses:
            return {'interpen': torch.tensor(0., device=self.device)}
        return {'interpen': torch.stack(all_pen_losses).mean()}
    ...
    def get_loss(self, loss, outputs, targets, indices, num_instances, **kwargs):
        loss_map = {
            ...
            'fov': self.loss_fov,
            'interpen': self.loss_interpenetration,
        }
