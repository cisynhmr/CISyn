...
class Model(nn.Module):
    ...
    def __init__(self, encoder, decoder,
        ...
        # per-joint heteroscedastic uncertainty (Kendall & Gal, Laplacian NLL)
        # num_sigma_joints should match the joint count output by process_smpl (num_kpts=45)
        # so that every joint contributing to the L1 loss has its own uncertainty.
        self.use_uncertainty = use_uncertainty
        self.num_sigma_joints = num_sigma_joints
        if self.use_uncertainty:
            sigma_head = MLP(hidden_dim, hidden_dim // 2, num_sigma_joints, 3)
            nn.init.zeros_(sigma_head.layers[-1].weight)
            nn.init.zeros_(sigma_head.layers[-1].bias)  # init → sigma=0 → neutral weight at start
            self.sigma_head = _get_clones(sigma_head, num_dec_layers)
    ...
    def forward(self, samples: NestedTensor, targets, sat_use_gt = False, detach_j3ds = False):
        ...
        for lvl in range(hs.shape[0]):
            ...
            if self.training or lvl == hs.shape[0] - 1:
                ...
                # outputs_shape = outputs_shape[:, :, :10]
                ...
                if self.use_uncertainty:
                    sigma = self.sigma_head[lvl](hs[lvl]).clamp(-6, 6)  # (B, nq, num_sigma_joints)
                    outputs_sigmas.append(sigma)
        ...
        pred_sigmas = torch.stack(outputs_sigmas) if self.use_uncertainty else None
        ...
        # pose uncertainty: std of cumulative pose_6d across decoder layers, mean over pose dims
        # shape: (bs, num_queries) — higher = decoder kept revising, lower = converged confidently
        pose_layer_std = torch.stack(all_pose_6d_lvls).std(dim=0).mean(dim=-1)
        ...
        out = {'pred_poses': pred_poses[-1], 'pred_betas': pred_betas[-1],
               ...
               'pose_layer_std': pose_layer_std}
        if self.use_uncertainty:
            out['pred_sigma'] = pred_sigmas[-1]  # (B, nq, 24) final-layer log_sigma
    ...
    def _set_aux_loss(self, pred_poses, pred_betas, pred_boxes,
                        ...
                        pred_j2ds, pred_depths, pred_sigmas=None):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        base = [{'pred_poses': a, 'pred_betas': b,
        ...
        if pred_sigmas is not None:
            for d, s in zip(base, pred_sigmas[:-1]):
                d['pred_sigma'] = s
        return base
...
def build_sat_model(args, set_criterion=True):
    ...
    model = Model(
        ...
        num_sigma_joints=getattr(args,'num_sigma_joints',45)
    )
    ...
    if set_criterion:
        ...
        criterion = SetCriterion(matcher, weight_dict, losses=losses, j2ds_norm_scale=args.input_size, detect_all_mask_lvl=args.det_mask_lvl, use_kid=getattr(args,'use_kid',False), smpl_faces=model.human_model.faces, use_j_occ_gt=getattr(args,'use_j_occ_gt',False))
