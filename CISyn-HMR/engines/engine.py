...
class Engine():
    def __init__(self, args, mode='train'): 
        ...
        self.eval_func_maps = {
            ...
            'chi3d_train': evaluate_3dpw,
            'hi4d_test': evaluate_3dpw,
            'cisyn_test': evaluate_3dpw,
        }
        ...
        if self.mode == 'train':
            ...
            self.do_tto = getattr(args, 'do_tto', False)
            self.tto_iters = getattr(args, 'tto_iters', 20)
            self.tto_lr = getattr(args, 'tto_lr', 1e-3)
        elif self.mode == 'eval':
    ...
    def prepare_models(self, args):
        ...
        if args.pretrain:
            ...
            if args.use_kid:    
                ...
                if pretrained_out_dim == 10:
                    ...
                    state_dict = {k: v for k, v in state_dict.items() if not k.startswith('human_model.') or k.startswith('mean_shape.')}
        ...
        if getattr(args, 'freeze_encoder', False):
            self.accelerator.print('Freezing module: encoder')
            for p in self.unwrapped_model.encoder.parameters():
                p.requires_grad_(False)
    ...
    def prepare_datas(self, args):
        ...
        if self.mode != 'infer':
            ...
            eval_ds = {f'{ds}_{split}': datasets_dict[ds](split = split, 
                                                          ...
                                                          force_aspect_ratio=getattr(args, 'force_aspect_ratio', None))\
    ...
    def prepare_training(self, args):
        ...
        if args.lr_scheduler == 'cosine':
            self.lr_scheduler = get_scheduler(name="cosine", optimizer=self.optimizer, 
                                          ...
                                          num_training_steps=4 * self.num_epochs * len(self.train_dataloader))
